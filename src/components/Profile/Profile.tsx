  ...
  return (
    <div className={style.profile}>
      <div className={style.profileContainer} style={profileContainerStyle} onClick={handleShowMenu}>
        {user?.photoURL ? <Image alt='profile' width={40} height={40} src={user?.photoURL} /> : <ProfileIcon width={`40`} height={`40`} />}
        <IoMdArrowDropdown className={`${style.arrow} ${showMenu && style.activeArrow}`}/>
      </div>
      {showMenu &&
        <div className={style.profileMenu} ref={menuRef}>
          <div className={style.profileName}>
            {user?.photoURL ? <Image alt='profile' width={24} height={24} src={user?.photoURL} /> : <ProfileIcon width={`24`} height={`24`} /> }
            <span>
